...
vopt_enable() {
    local opt="$1" flag="${2:-$1}"
    if [ "$#" -gt "2" ]; then
        echo "vopt_enable $opt: $(($# - 2)) excess parameter(s)\n"
    fi
...
vopt_conflict() {
    ...
    if [ "${!n1}" -a "${!n2}" ]; then
        echo "options '${opt1}' and '${opt2}' conflict\n"
    fi
}
...
vopt_bool() {
    ...
    if [ "$#" -gt "2" ]; then
        echo "vopt_bool $opt: $(($# - 2)) excess parameter(s)\n"
    fi
...
vopt_feature() {
    local opt="$1" prop="${2:-$1}"
    if [ "$#" -gt "2" ]; then
        echo "vopt_feature $opt: $(($# - 2)) excess parameter(s)\n"
    fi
